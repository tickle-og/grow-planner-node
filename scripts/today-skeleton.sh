...
<div class="today mx-auto px-4 py-6">
  ...
  <section aria-label="Status overview" class="grid grid-cols-2 md:grid-cols-4 gap-3">
    {#each [
      { label: 'Pending',   value: sc.pending },
      { label: 'Active',    value: sc.active },
  ...
  <section class="mt-8">
    ...
    {:else}
      <ul class="divide-y divide-neutral-800 rounded-lg border border-neutral-800 bg-neutral-900/80">
        {#each actions as a}
          <li class="flex items-center gap-3 p-3">
            ...
            <button
              ...
              on:click={() => console.log('TODO: dismiss', a?.id)}
            >Dismiss</button>
          </li>
        {/each}
      </ul>
  ...
  <section class="mt-10">
    <div class="rounded-lg border border-neutral-800 bg-neutral-900/80 p-3">
      ...
      <pre class="text-xs text-neutral-200 overflow-auto"><code>curl -X POST http://localhost:5173/api/dev/seed/default-location \
