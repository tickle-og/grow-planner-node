...
<script lang="ts">
  ...
  async function loadAll() {
    ...
    try { lowStock = await fetch(`/api/dashboard/low-stock?${params}`).then(j); } catch {}
    try { recentYields = await fetch(`/api/dashboard/recent-yields?${params}`).then(j); } catch {}
    try { shelfUtil = await fetch(`/api/dashboard/shelf-util?${params}`).then(j); } catch {}
    try { nextActions = await fetch(`/api/dashboard/next-actions?${params}`).then(j); } catch {}
  }
  loadAll();
</script>
...
<style>
  .reports-page { --card-bg:#0B1020; --ink:#E6EAF2; --muted:#9AA4B2; }
  .reports-page { background: #070B16; color: var(--ink); min-height: 100%; padding-bottom: 4rem; }
  .card { background: var(--card-bg); border: 1px solid rgba(255,255,255,0.08); border-radius: 14px; padding: 16px 16px 12px; }
  h2 { font-weight: 700; font-size: 14px; letter-spacing: .04em; text-transform: uppercase; color: var(--ink); opacity: .9; margin-bottom: 8px; }
  ul { margin: 0; padding-left: 1rem; }
  li { color: var(--ink); opacity: .95; }
