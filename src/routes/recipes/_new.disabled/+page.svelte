<script lang="ts">
	...
	const exampleSteps = [
		...
		{
			key: 'sterilize_grain',
			title: 'Sterilize grain (PC 120m)',
			duration: '2h',
			depends_on: ['hydrate_grain']
		},
	...
	<form method="POST" action="?/create" class="grid gap-4">
		<div>
			...
			<input
				...
				placeholder="Monotub 3.5 lb Flow"
			/>
		</div>
		...
		<div class="grid grid-cols-2 gap-4">
			...
			<div>
				...
				<textarea
					name="media_json"
					class="w-full h-[110px] px-3 py-2 rounded bg-neutral-900 border border-neutral-700"
					>{JSON.stringify(exampleMedia, null, 2)}</textarea
				>
			</div>
		</div>
		...
		<div>
			...
			<textarea
				name="steps_json"
				class="w-full h-60 px-3 py-2 rounded bg-neutral-900 border border-neutral-700"
				>{JSON.stringify(exampleSteps, null, 2)}</textarea
			>
			<p class="text-xs text-neutral-500 mt-1">
				Array of steps with <code>key</code>, <code>title</code>, <code>duration</code> (e.g.,
				"14d", "120m"), and optional <code>depends_on</code>.
			</p>
		</div>
