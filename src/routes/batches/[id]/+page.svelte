    ...
    <div>
      ...
      <form method="POST" action="?/add_log" class="grid gap-3">
        <div>
          ...
          <select name="kind" class="w-full px-3 py-2 rounded bg-neutral-900 border border-neutral-700">
        ...
        <div>
          ...
          <textarea name="text" rows="2" class="w-full px-3 py-2 rounded bg-neutral-900 border border-neutral-700"></textarea>
        </div>
        <div class="grid grid-cols-3 gap-2">
          <div>
            <label class="block text-xs text-neutral-400 mb-1">Temp (°F/°C)</label>
            <input name="temp" type="number" step="0.1" class="w-full px-2 py-1 rounded bg-neutral-900 border border-neutral-700" />
          </div>
          <div>
            <label class="block text-xs text-neutral-400 mb-1">RH (%)</label>
            <input name="rh" type="number" step="0.1" class="w-full px-2 py-1 rounded bg-neutral-900 border border-neutral-700" />
          </div>
          <div>
            <label class="block text-xs text-neutral-400 mb-1">CO₂ (ppm)</label>
            <input name="co2" type="number" step="1" class="w-full px-2 py-1 rounded bg-neutral-900 border border-neutral-700" />
          </div>
        </div>
        <div class="flex gap-2">
          <button class="px-3 py-2 rounded bg-emerald-600 hover:bg-emerald-700" type="submit">Add Log</button>
        </div>
      </form>
    </div>
        ...
        <ul class="space-y-2">
          {#each data.logs as l}
            <li class="border border-neutral-800 rounded p-2">
              ...
              <pre class="text-sm bg-neutral-900 border border-neutral-800 rounded p-2 overflow-auto">{l.payload}</pre>
            </li>
          {/each}
        </ul>
      {/if}
      ...
      <form method="POST" action="?/add_yield" class="grid gap-3">
        <div class="grid grid-cols-3 gap-2">
          <div>
            ...
            <input name="flushNo" type="number" min="1" value="1" class="w-full px-2 py-1 rounded bg-neutral-900 border border-neutral-700" />
          </div>
          <div>
            <label class="block text-xs text-neutral-400 mb-1">Wet Weight (g)</label>
            <input name="wetWeightG" type="number" min="0" step="1" class="w-full px-2 py-1 rounded bg-neutral-900 border border-neutral-700" />
          </div>
          <div>
            <label class="block text-xs text-neutral-400 mb-1">Dry Weight (g)</label>
            <input name="dryWeightG" type="number" min="0" step="1" class="w-full px-2 py-1 rounded bg-neutral-900 border border-neutral-700" />
          </div>
        </div>
        <div>
          <label class="block text-sm text-neutral-400 mb-1">Notes</label>
          <textarea name="notes" rows="2" class="w-full px-3 py-2 rounded bg-neutral-900 border border-neutral-700"></textarea>
        </div>
        <div class="flex gap-2">
          <button class="px-3 py-2 rounded bg-emerald-600 hover:bg-emerald-700" type="submit">Add Yield</button>
        </div>
      </form>
    </div>
        ...
        <ul class="space-y-2">
          {#each data.yields as y}
            <li class="border border-neutral-800 rounded p-2">
              ...
              <div class="text-sm">Flush #{y.flushNo} · Wet: {y.wetWeightG}g {#if y.dryWeightG}· Dry: {y.dryWeightG}g{/if}</div>
