...
export const load: PageServerLoad = async ({ params }) => {
	...
	const tasks = await db
		.select()
		.from(schema.tasks)
		.where(eq(schema.tasks.batchId, id))
		.orderBy(asc(schema.tasks.dueAt))
		.all();
	const logs = await db
		.select()
		.from(schema.logs)
		.where(eq(schema.logs.batchId, id))
		.orderBy(desc(schema.logs.createdAt))
		.all();
	const yields = await db
		.select()
		.from(schema.yieldsTbl)
		.where(eq(schema.yieldsTbl.batchId, id))
		.orderBy(desc(schema.yieldsTbl.createdAt))
		.all();
...
export const actions: Actions = {
	...
	add_yield: async ({ request, params }) => {
		const batchId = params.id!;
		const form = await request.formData();
