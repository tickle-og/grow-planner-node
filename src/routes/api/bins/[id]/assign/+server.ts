...
export const POST: RequestHandler = async ({ params, request }) => {
  ...
  try {
    ...
    if (!growId) return new Response(JSON.stringify({ ok: false, error: 'growId required' }), { status: 400 });

    // Check bin exists + belongs to location
    const [bin] = await db.select().from(shelfBins).where(eq(shelfBins.id, binId)).limit(1);
    if (!bin) return new Response(JSON.stringify({ ok: false, error: `bin ${binId} not found` }), { status: 404 });
    if (bin.locationId !== locationId) {
      return new Response(JSON.stringify({ ok: false, error: 'bin belongs to a different location' }), { status: 409 });
    }

    // Check grow exists + belongs to location
    const [grow] = await db.select().from(grows).where(eq(grows.id, Number(growId))).limit(1);
    if (!grow) return new Response(JSON.stringify({ ok: false, error: `grow ${growId} not found` }), { status: 404 });
    if (grow.locationId !== locationId) {
      return new Response(JSON.stringify({ ok: false, error: 'grow belongs to a different location' }), { status: 409 });
    }
