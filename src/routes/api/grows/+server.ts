...
export const POST: RequestHandler = async ({ request }) => {
	try {
		...
		if (!createdByUserId)
			return new Response(JSON.stringify({ ok: false, error: 'createdByUserId required' }), {
				status: 400
			});
		...
		const [usr] = await db
			.select()
			.from(users)
			.where(eq(users.id, Number(createdByUserId)))
			.limit(1);
		if (!usr)
			return new Response(
				JSON.stringify({ ok: false, error: `user ${createdByUserId} not found` }),
				{ status: 404 }
			);

		const now = new Date().toISOString();
		const cfg =
			containerConfig && typeof containerConfig !== 'string'
