  ...
  try {
    ...
    if (shelfId != null) {
      const [shelf] = await db
        ...
        .limit(1);
      if (!shelf) return new Response(JSON.stringify({ ok: false, error: `shelf ${shelfId} not found` }), { status: 404 });
      if (shelf.locationId !== locationId) {
        return new Response(JSON.stringify({ ok: false, error: 'shelf belongs to a different location' }), { status: 409 });
      }
    }
