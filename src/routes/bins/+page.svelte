...
<div class="max-w-4xl mx-auto p-4">
  <div class="flex items-center justify-between mb-4">
    ...
    <button class="border rounded px-3 py-2 text-sm" on:click={refreshAll} aria-label="Refresh bins and shelves">
      Refresh
    </button>
  </div>
  ...
  <form class="grid gap-3 mb-6" on:submit|preventDefault={createBin}>
    ...
    <div class="grid grid-cols-3 gap-3">
      <div>
        ...
        <input id="cap" type="number" class="w-full border rounded px-3 py-2" bind:value={form.capacityCm2} min="0" />
      </div>
    ...
    <button class="inline-flex items-center gap-2 border rounded px-4 py-2" disabled={!form.label} on:click|preventDefault={createBin}>
      Create bin
    </button>
  </form>
    ...
    {#each bins as b}
      <div class="border rounded p-3">
        <div class="flex items-center justify-between">
          <div>
            <div class="font-medium">{b.label} <span class="text-xs text-gray-500">#{b.id}</span></div>
