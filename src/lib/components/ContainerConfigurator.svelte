<script lang="ts">
	...
	async function loadCatalog() {
		const [p, j] = await Promise.all([
			fetch('/api/catalog/container-presets')
				.then((r) => r.json())
				.then((rows) => rows.map((r: any) => ({ ...r, defaults: JSON.parse(r.defaultsJson) }))),
	...
	function emit() {
		dispatch('change', {
			...
			containerPresetKey: selectedPreset,
			containerConfig: cfg,
			jarVariantId
		});
	}
...
<div class="space-y-3">
	...
	{#if selectedType && selectedType !== 'jar'}
		...
		{#if selectedType === 'monotub' || selectedType === 'tray'}
			<div class="grid grid-cols-3 gap-3 mt-2">
				<div>
					<label class="text-xs">Length (cm)</label><input
						class="border p-2 w-full"
						type="number"
						bind:value={cfg.length_cm}
					/>
				</div>
				<div>
					<label class="text-xs">Width (cm)</label><input
						class="border p-2 w-full"
						type="number"
						bind:value={cfg.width_cm}
					/>
				</div>
				<div>
					<label class="text-xs">Height (cm)</label><input
						class="border p-2 w-full"
						type="number"
						bind:value={cfg.height_cm}
					/>
				</div>
			</div>
			<div class="mt-2">
		...
		{#if selectedType === 'bag'}
			<div class="grid grid-cols-3 gap-3 mt-2">
				<div>
					<label class="text-xs">Volume (L)</label><input
						class="border p-2 w-full"
						type="number"
						step="0.1"
						bind:value={cfg.volume_l}
					/>
				</div>
				<div>
					<label class="text-xs">Patch (mm)</label><input
						class="border p-2 w-full"
						type="number"
						bind:value={cfg.filter_patch_size_mm}
					/>
				</div>
				<div>
					<label class="text-xs">Thickness (mil)</label><input
						class="border p-2 w-full"
						type="number"
						bind:value={cfg.thickness_mil}
					/>
				</div>
			</div>
		{/if}
	...
	{#if selectedType === 'jar'}
		...
		<div class="grid grid-cols-3 gap-3 mt-2">
			...
			<div>
				<label class="text-xs">Mouth</label>
